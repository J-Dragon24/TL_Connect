...
CREATE TABLE student_course_class_logs (
  ...
  student_id BIGINT NOT NULL,
  course_class_id BIGINT NOT NULL,
  action VARCHAR(20) NOT NULL DEFAULT 'REJECTED' CHECK (action IN ('ADD', 'DROP', 'REJECTED')),
  from_status VARCHAR(20) NOT NULL DEFAULT 'PENDING' CHECK (from_status IN ('PENDING', 'ENROLLED', 'DROPPED', 'REJECTED')),
